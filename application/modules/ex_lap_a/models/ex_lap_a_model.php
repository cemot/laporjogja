<?php
class ex_lap_a_model extends CI_Model {
...
function data($param){
	...
	if($param['jenis']<>'x'  ) {
		...
		if($param['jenis']=="polres") {
			$this->db->where("v_lap_aa.id_polres",$param['id_polres']);
			$this->db->where("v_lap_aa.jenis",$param['jenis']);
		}
		else if($param['jenis']=="polsek") {
			$this->db->where("v_lap_aa.id_polsek",$param['id_polsek']);
			$this->db->where("v_lap_aa.jenis",$param['jenis']);
		}
