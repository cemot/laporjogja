<?php
class admin_fungsi extends admin_controller {

	var $controller ;

	function admin_fungsi(){
		parent::__construct();
		// $this->load->model("core_model","cm");
		$this->load->model("coremodel","cm");
		$this->load->helper("tanggal");
		$this->load->model("admin_fungsi_model","dm");
	...
	function index(){
		...
		$this->set_subtitle("DATA FUNGSI TERKAIT");
		$this->set_title("DATA FUNGSI TERKAIT");
...
function simpan(){
		...
		if($this->form_validation->run() == TRUE ) { 
			...
			unset($data['id_fungsi']);

			//$data['id_kesatuan'] = md5(microtime()); 
			 

			 
			 $res = $this->db->insert("m_fungsi",$data);
			 if($res) {
...
function update(){
		...
		if($this->form_validation->run() == TRUE ) { 
			 ...
			 $res = $this->db->update("m_fungsi",$data);
			 if($res) {
...
function hapus(){
	$data = $this->input->post();
	$this->db->where("id_fungsi",$data['id']);
	$res = $this->db->delete("m_fungsi");
	if($res){
