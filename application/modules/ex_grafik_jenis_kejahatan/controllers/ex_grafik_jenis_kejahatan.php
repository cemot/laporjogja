<?php
class ex_grafik_jenis_kejahatan extends super_controller  {
	function ex_grafik_jenis_kejahatan(){
		parent::__construct();
	...
	function index(){
		...
		$this->db->where("id_kelompok in (1,2,3,4,5)",null,false);
...
from 
	...
	join pengguna u on u.id = x.user_id 
		...
		$sql .= " where year(tanggal) = '$tahun' 	
				and mk.id_kelompok = '$id_kelompok' 
		";
		...
		$sql .= " group by a.id 
				order by count(x.id_gol_kejahatan) desc limit 10";
		...
		$data['data'] = $res;
		$data['title'] = "GRAFIK JENIS  KEJAHATAN ";
