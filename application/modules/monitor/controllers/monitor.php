...
class monitor extends CI_Controller {
	...
	function ceklogin(){
		...
		$this->db->where("mon_user",$mon_user); 
		$this->db->where("md5(mon_pass) = '$mon_pass'",null,false); 
		//$this->db->get('lap_b');

		$res = $this->db->get('lap_b'); 

		if($res->num_rows()==1) { // berhasil 
			$data = $res->row(); 

			$_SESSION['monitor'] = true;
			$_SESSION['lap_b_id'] = $data->lap_b_id;
			$ret = array("error"=>false,"message"=>"Login berhasil","url"=>site_url("monitor_dashboard"));
